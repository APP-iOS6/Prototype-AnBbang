# 여성 전용 안심 월세방 비교 서비스 "AnBbang"

<img src = "https://github.com/user-attachments/assets/10db4085-0023-4b16-94d7-ef9bf7196495" width="350" height="100">

<br> 

## ✅ 소개 (Introduction)
'AnBbang(안방)'은 20대 여성(대학생, 직장인)을 주 대상으로 안전하고 신뢰할 수 있는 '월세방 비교 서비스'를 제공합니다. <br>
사용자가 쉽게 검증된 여성 전용 매물을 찾을 수 있도록 하고, 사기 피해를 방지하며 안전한 주거 환경을 보장하는데 중점을 둡니다. <br>
또한, 경찰서 및 CCTV 위치, 여성 전용 피트니스 센터 및 여성 병원 등 '안전'과 '편의' 관련 정보도 함께 제공합니다. <br><br> 
안전하고 안심할 수 있는(安) 주거(房) 정보를 제공하는 것이 우리 'AnBbang(안방, 安房)'의 핵심 가치입니다. <br>
<br>

### ☑️ ADS
- <b>20대 여성</b>(대학생, 직장인 등)<b>을 주 대상으로 안전</b>(치안)<b>, 사기 등으로부터 검증된 '여성 전용 안심 월세방 비교 서비스' 제공 </b>
<br>

### ☑️ 페르소나 (Personas)
- <img src = "https://github.com/user-attachments/assets/1ce9f2c9-4567-4c07-b282-5bec2a0456f7" width="700" height="400">
- <img src = "https://github.com/user-attachments/assets/0bd6a7f0-5125-4e11-b946-7528bb662ea1" width="700" height="370">
<br>

### ☑️ 주요기능
- 카테고리별 검색: ⓵ 여성 전용 고시텔, ⓶ 원룸, ⓷ 투룸 및 빌라, ④ 오피스텔의 월세방 매물 정보 제공
- 지도를 통해 주거지 인근 '경찰서', 'CCTV', '여성 전용 시설' 위치 정보 제공
  * 월세방 매물 정보와 함께 '안전(경찰서 및 지구대', 편의(여성 전용 피트니스 센터, 여성 병원)' 시설 위치 가시화
- 가격 필터를 통해 '최저가' 및 '여성 전용' 매물 정보 제공
- '사용자 리뷰 및 평점'을 통해 '검증된' 매물 정보 제공, 과장 및 허위매물 방지
- 매물(월세방) 상세 정보 제공

<br>

### ☑️ 시나리오
- <img src = "https://github.com/user-attachments/assets/0ef5b148-23bb-407f-8be8-86dc9c0f9f8c" width="700" height="370">

<br>

## ✅ 기능 설명 (Features)

0. 로그인 화면
   - 간편 로그인 및 이메일 로그인 후 서비스 이용

<br>

1. 홈 화면
   - '여성 전용 고시텔', '원룸', '투룸 및 빌라', '오피스텔' 중 원하는 카테고리 선택
   - 수익화 위한 광고 배너 운영
   - '추천 매물' 기능

<br>
  
2. 지도 뷰
   - 선택한 카테고리의 매물과 함께 경찰서, CCTV, 여성 전용 시설(피트니스 센터, 여성 병원) 위치 확인
   - 관심 지역의 매물을 '가격순', '여성 전용'으로 선택적 정보 확인 가능

<br>
     
3. 매물 비교
   - 매물 가격, 위치, 주변 안전시설 편의시설을 비교하여 '가장 안전하고 적합한 매물' 선택 가능
   - '계약 전, 중, 후' 해야 할 내용을 제공하여 사용자의 궁금증 및 불안감 해소
   - '관심목록'을 통해 '나의 관심 매물' 관리
   - 공인중개소 문의 기능

<br>
     
4. 사용자 리뷰 및 평점
   - 실제 해당 월세방을 사용했던 '사용자 리뷰 및 평점'을 통해 검증된 매물 선택 가능

<br>

5. '더보기 기능
   - 사용자 정보 관리
   - '방 내놓기' 기능: 공인중개사 활용 서비스
   - '문의한 방', '내가 쓴 이야기' 등을 통해 사용 기록 관리
   - '허위매물 신고 내역'을 통해 사용자 피해 방지
   - '이벤트', '새소식' 등의 알림 서비스
   - 로그아웃
<br>
<img src = "https://github.com/user-attachments/assets/eb139f43-5730-4e9b-8e96-25f0369ca19c" width="250" height="550">
<img src = "https://github.com/user-attachments/assets/fb2fdf95-fb96-44e9-8714-41b78725c441" width="250" height="550">

<br><br> 

## ✅ 피드백 / 개선 사항 (Feedback / Improvements)
### ☑️ Advice (feat. Other Team)
- "(여성을 위한 서비스인데) 남자도 가입이 가능한지?" <br>
  ➡️ 로그인 화면 및 기능 수정: '여성사용자만 가입이 가능함'을 공지 <br>
  ➡️ 하단에 '공인중개사 로그인' 버튼 추가
<br>

- "로그인 후 '알림' 허용 여부를 물어볼 때, 마케팅적으로 부정적인 감정 유발 가능성이 있어 보임." <br>
  ➡️ '알림 허용 여부' 문구 수정
<br>

- "홈 화면 내 '광고'의 위치와 크기 적절성 검토" <br>
  ➡️ 위치는 그대로이나 크기와 비율 조정, 주제와 부합되는 광고 반영
<br>

- "'홈' 화면 하단의 '이 근처 추천 매물'에 매물 리스트를 추가로 더 볼 수 있는 기능 추가" <br>
  ➡️ '더보기' 기능 터치 시, '추천 매물 리스트' 제공 기능 반영
<br>

- "안전 및 편의 기능을 조금 더 부각시켰으면 좋겠음." <br>
  ➡️ 지도뷰 내 '편의' 기능 추가: 안심택배, 빨래방 <br>
  ➡️ 매물 세부정보 내에 '안전' 및 '편의' 정보 추가 <br>
  ➡️ '성범죄자 알림e' 사이트 링크 제공('더보기' 기능에 추가) <br> 
  ➡️ '보안' 기능 추가: 경찰서 위치 + 공공기관 및 안전시설 추가 반영 (가용 시, 추후 개발 예정)
<br>

- "지도뷰 하단의 '매물 정보 시트'의 노출 시점 및 방식에 대한 재검토" <br>
  ➡️ 지도뷰 화면 '터치' 시, 하단의 '매물 정보 시트'가 노출되도록 변경 <br>
  ➡️ '매물 정보 시트' 내 '더보기' 기능 추가
<br>

- "매물 세부정보 내 '사용자 리뷰 보기' 가시화 및 구체화" <br>
  ➡️ '리뷰 보기' 텍스트 크기 확대 <br>
  ➡️ '방 리뷰' 내 사진 크기 확대, 사용자가 리뷰에 '실 사용 사진'을 추가할 수 있도록 반영
<br>

- "매물의 세부 정보를 '하이픈'을 넣어 구분해줬으면 좋겠음." <br>
  ➡️ 반영 완료
<br>

- "(현재 구현되어 있는 로그아웃 기능의) '로그아웃 버튼' 추가" <br>
  ➡️ '더보기' 메뉴 하단에 '로그아웃' 버튼 추가
<br>

- "계약기간이 남은 상태로 이사를 가야하는 세입자의 경우, '양도' 관련 커뮤니티 기능이 있었으면 좋겠음." <br>
  ➡️ 가용 시, 추후 개발 예정
<br>

- (개발팀의 추가 아이디어) "사용자가 원하는 매물의 가격, 별점 등을 설정 후 원하는 매물이 등록될 시 '알림' 기능" <br>
  ➡️ 가용 시, 추후 개발 예정
<br>


### ☑️ UX Feedback (feat. Tuna)
- "지도뷰 좌측 상단의 '주거 형태' 버튼의 애니메이션 수정" <br>
  ➡️ 수정 완료
  
<br>

- "지도뷰 하단 '매물 리스트' 필터 중 '여성 전용' 레이블 사용여부 재검토, '회원 리뷰가 달린 매물' 카테고리 사용 검토" <br>
  ➡️ '여성 전용' 레이블은 그대로 사용하되 '회원 리뷰 매물' 카테고리 추가
  
<br>

- "'매물 세부 정보'뷰 내 정보의 위계를 고려한 보완, '사용자 리뷰 창' 내 스크롤뷰 높이가 너무 낮음." <br>
  ➡️ '매물 등록번호', '매물 등록일', '사용자 리뷰 및 별점' 등의 위치 재조정 / '사용자 리뷰' 내 스크롤뷰 높이 수정

<br>

## ✅ 작동환경 (Operating Environment)
- Xcode 16.0
- iOS 18.0

<br>

## ✅ 개발자 (Developer)

| 이름     | 역할  | GitHub |
|----------|-------|--------|
| 김승철 | PM, 로그인, 디자인  | [![GitHub](https://img.shields.io/badge/GitHub-%2312100E.svg?style=flat-square&logo=github&logoColor=white)](https://github.com/Seungcheol-Kim) |
| 박범규 | 관심목록, 매물정보  | [![GitHub](https://img.shields.io/badge/GitHub-%2312100E.svg?style=flat-square&logo=github&logoColor=white)](https://github.com/parkbeomgyu) |
| 신현우 | 홈, 총괄   | [![GitHub](https://img.shields.io/badge/GitHub-%2312100E.svg?style=flat-square&logo=github&logoColor=white)](https://github.com/show2633) |
| 황인영 | 더보기, 사용자 리뷰 | [![GitHub](https://img.shields.io/badge/GitHub-%2312100E.svg?style=flat-square&logo=github&logoColor=white)](https://github.com/InyoungTechit) |
| 홍재민 | 지도뷰 | [![GitHub](https://img.shields.io/badge/GitHub-%2312100E.svg?style=flat-square&logo=github&logoColor=white)](https://github.com/IUCyH) |

<br>

## ✅ 라이선스
Licensed under the [MIT](LICENSE) license.
